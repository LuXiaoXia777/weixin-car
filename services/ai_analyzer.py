...
class TomorrowSuggestion(BaseModel):
    title: str
    reason: str
    reference_data: str
...
class AIAnalysisResult(BaseModel):
    ...
    content_trend: str
    problems: str
    weekly_content_patterns: list[str] = Field(min_length=3, max_length=3)
    tomorrow_suggestions: list[TomorrowSuggestion] = Field(min_length=3, max_length=3)
...
3. 最佳文章必须来自 top5_articles，理由必须引用真实标题和指标。
4. weekly_content_patterns 必须分别说明：表现最好的标题规律、平均阅读最高的主题、应该减少的内容；每条引用具体数字，证据不足就明确说明。
5. 明日建议必须给出可直接使用的汽车公众号标题，并填写推荐原因、参考数据、预计表现。
6. 预计表现只能做基于历史数据的区间或相对判断，数据不足时写“数据不足，暂无法判断”，禁止承诺阅读量。
7. 使用简洁中文，不输出 Markdown，不输出 JSON 以外的内容。
"""
